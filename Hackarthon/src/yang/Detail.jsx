...
const Detail = () => {
  ...
  const [activePlaceId, setActivePlaceId] = useState(
    routeData.length > 0 ? routeData.at(0)?.id : null
  );
...
<<<<<<< HEAD
  ...
  const handlePlaceClick = (id) => {
    setActivePlaceId(id);
    const placeElement = placeRefs.current.find(
      (ref) => ref?.dataset?.id === id.toString()
    );
        ...
        <div
          ...
          ref={(el) => {
            if (el) {
              ...
              if (
                !placeRefs.current.some(
                  (ref) => parseInt(ref?.dataset?.id) === place.id
                )
              ) {
